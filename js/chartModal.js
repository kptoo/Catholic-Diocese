const ChartModal = {
    ...
    open(dioceseInfo) {
        ...
        document.getElementById('modal-diocese-subtitle').textContent = 
            `${country} • ${CONFIG.STATISTICS[statistic].label} • ${historicalData.length} years of data`;
    ...
    renderModalChart(data, statistics) {
                ...
                <div style="display: flex; align-items: center; justify-content: center; height: 100%; flex-direction: column;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; width: 100%; max-width: 800px;">
                        ${statistics.map(stat => {
                            ...
                            return `
                                <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); padding: 24px; border-radius: 8px; border-left: 4px solid #4a90e2; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                                    <div style="font-size: 14px; color: #666; margin-bottom: 8px; font-weight: 600;">${stat}</div>
                                    <div style="font-size: 28px; font-weight: 700; color: #2c3e50;">${formatted}</div>
                                </div>
                            `;
                        }).join('')}
                    </div>
        ...
        const colorScale = d3.scaleOrdinal()
            .domain(statistics)
            .range(['#4a90e2', '#e74c3c', '#2ecc71', '#f39c12', '#9b59b6', '#1abc9c']);
